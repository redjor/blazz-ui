...
const badgeVariants = cva("inline-flex items-center justify-center whitespace-nowrap shrink-0 transition-colors [&>svg]:pointer-events-none group/badge rounded-[var(--badge-radius)] font-medium", {
	variants: {
		variant: {
			default: "",
			secondary: "",
...
function Badge({ className, variant = "default", size = "sm", fill = "solid", dot, onDismiss, children, render, ...props }: BadgeProps) {
						...
						{dot && (
							<span
								className={cn(
									...
									dotSizeMap[sizeKey],
									fillKey === "solid" ? dotSolidColorMap[variantKey] : fillKey === "ghost-dot" || fillKey === "ghost" ? dotSemanticColorMap[variantKey] : dotColorMap[variantKey]
								)}
							/>
						)}
						{children}
