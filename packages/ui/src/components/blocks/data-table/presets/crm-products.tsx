...
export function createProductsPreset(config: ProductsPresetConfig = {}): ProductsPreset {
	...
	if (onDeactivate) {
		customActions.push(
			createCustomRowAction<Product>({
				...
				confirmationMessage: (product) => `Êtes-vous sûr de vouloir désactiver "${product.name}" ?`,
			})
		)
	}
	...
	const bulkActions = createBulkActions<Product>({
		...
		archiveConfirmation: (count) => `Êtes-vous sûr de vouloir désactiver ${count} produit(s) ?`,
		deleteConfirmation: (count) => `Êtes-vous sûr de vouloir supprimer ${count} produit(s) ?`,
		labels: {
