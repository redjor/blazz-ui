...
export function DataTableRowActions<TData>({ row, actions }: DataTableRowActionsProps<TData>) {
	...
	return (
		<>
			<DropdownMenu>
				<DropdownMenuTrigger
					className="inline-flex items-center justify-center whitespace-nowrap rounded-lg text-sm font-medium transition-colors hover:bg-surface-3 hover:text-fg h-8 w-8 p-0"
							...
							return (
								<React.Fragment key={action.id}>
									...
									<DropdownMenuItem
										...
										disabled={isDisabled}
										className={
											action.variant === "destructive" ? "text-negative focus:text-negative" : ""
										}
									>
