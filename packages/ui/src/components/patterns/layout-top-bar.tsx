...
export function LayoutTopBar({ onOpenCommandPalette, className }: LayoutTopBarProps) {
	return (
		<header className={cn("flex h-14 items-center justify-between bg-fg px-4", className)}>
			...
			{/* Center: Search Bar */}
			<button type="button" onClick={onOpenCommandPalette} className="mx-4 flex max-w-md flex-1 items-center gap-2 rounded-lg bg-[#464C53] px-3 py-1.5 transition-colors hover:bg-[#505761]">
			...
			<div className="flex items-center gap-2">
				<button type="button" className="rounded-lg p-2 transition-colors hover:bg-[#464C53]" aria-label="Notifications">
					<Bell className="h-5 w-5 text-gray-300" />
				</button>
				<button type="button" className="flex items-center gap-2 rounded-lg p-2 transition-colors hover:bg-[#464C53]">
					<div className="flex h-6 w-6 items-center justify-center rounded-full bg-[#95BF47] text-xs font-semibold text-white">U</div>
