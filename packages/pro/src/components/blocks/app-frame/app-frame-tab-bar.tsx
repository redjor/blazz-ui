...
export function AppFrameTabBar({ mobileLogo, defaultNewTabUrl = "/", defaultNewTabTitle = "Home" }: AppFrameTabBarProps) {
	...
	return (
		<>
			...
			<div className="flex md:hidden h-10 items-center gap-2 border-b border-edge-subtle bg-muted px-3">
				<SidebarTrigger />
				{mobileLogo && <Link href="/">{mobileLogo}</Link>}
			</div>
				...
				<div className="flex items-center">
					...
					<TabsBar
						...
						renderDragOverlay={(dragId) => {
							...
							if (!tab) return null
							return <TabsItemOverlay title={tab.title} isActive={tab.id === activeTabId} />
						}}
						onAddTab={() => {
