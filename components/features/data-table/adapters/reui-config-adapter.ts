...
import type { FilterFieldConfig, FilterOperator as ReuiFilterOperator } from "@/components/ui/filters"
...
export function columnToFilterFieldConfig<TData>(
	...
	const isMultiselect =
		...
		(filterConfig.operators?.some((op) => op === "in" || op === "notIn") ?? true)
	...
	const config: FilterFieldConfig = {
		...
		searchable: filterConfig.type === "select" && filterConfig.options && filterConfig.options.length > 5,
	...
	if (filterConfig.type === "select" && filterConfig.options) {
		config.options = filterConfig.options.map((opt) => ({
			value: opt.value,
			label: opt.label,
		}))
	}
 ...
 */
export function separateFilterFields<TData>(
	columns: DataTableColumnDef<TData, any>[]
): {
