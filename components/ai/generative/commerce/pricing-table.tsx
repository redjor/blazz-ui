...
export function PricingTable({
	...
	return (
		<div className={cn("rounded-lg border border-edge bg-surface p-4", className)}>
			...
			<div className={cn("grid gap-3", title && "mt-3", tiers.length === 2 ? "grid-cols-2" : tiers.length >= 3 ? "grid-cols-3" : "grid-cols-1")}>
				{tiers.map((tier) => (
					<div
						key={tier.name}
						className={cn(
							...
							tier.recommended
								...
								: "border-edge-subtle bg-raised/50",
						)}
					>
						<div className="flex items-center gap-2">
							<span className="text-xs font-semibold text-fg">{tier.name}</span>
							{tier.recommended && (
								<Badge variant="default" size="xs" fill="subtle">Recommended</Badge>
							)}
						</div>
