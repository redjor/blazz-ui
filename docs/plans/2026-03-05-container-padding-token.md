...
**Goal:** Add a `--inset` CSS token and utility classes to unify padding across flat container components (Card, Dialog, Sheet, Popover, HoverCard, Alert).

**Architecture:** Single CSS custom property in `tokens.css` consumed via utility classes (`.p-inset`, `.px-inset`, etc.) and Tailwind arbitrary values for edge cases. Components replace hardcoded padding values with the token.
...
.pt-inset { padding-top: var(--inset); }
.-mx-inset { margin-inline: calc(-1 * var(--inset)); }
.-mt-inset { margin-top: calc(-1 * var(--inset)); }
.-mb-inset { margin-bottom: calc(-1 * var(--inset)); }
```
...
git commit -m "feat(ui): add --inset token and utility classes"
```
...
**Step 1: Update CardHeader — replace `px-4 pt-4` with `px-inset pt-inset`**
...
**Step 2: Update CardContent — replace `p-4` with `p-inset`**
...
**Step 3: Update CardFooter — replace `p-4` with `p-inset`**
...
git commit -m "feat(ui): migrate Card to --inset token"
```
...
**Step 1: Update DialogContent — replace `p-4` with `p-inset`**

In the long className string on line 62, replace `p-4` with `p-inset`:

```tsx
// before (extract):
"... rounded-xl p-4 text-sm ring-1 ..."
// after:
"... rounded-xl p-inset text-sm ring-1 ..."
```
...
git commit -m "feat(ui): migrate Dialog to --inset token"
```
...
git commit -m "feat(ui): migrate Sheet to --inset token"
```
...
**Step 1: Update PopoverContent — replace `p-2.5` with `p-inset`**

In the long className on line 43, replace `p-2.5` with `p-inset`:

```tsx
// before (extract):
"... rounded-lg p-2.5 text-sm ..."
// after:
"... rounded-lg p-inset text-sm ..."
```

**Step 2: Update HoverCardContent — replace `p-2.5` with `p-inset`**

In the long className on line 41, replace `p-2.5` with `p-inset`:
...
git commit -m "feat(ui): migrate Popover and HoverCard to --inset token"
```
...
// after:
"... rounded-lg border px-inset text-left ..."
```
...
git commit -m "feat(ui): migrate Alert to --inset token"
```
...
**Step 3: Test override — add `--inset: 0.5rem` via browser DevTools**
