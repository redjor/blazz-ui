			...
			{examples.map((example) => {
				...
				return (
					<button
						...
						className="w-full text-left border border-edge rounded-md p-3 hover:border-brand/50 hover:bg-raised cursor-pointer transition-colors"
					>
						<p className="text-sm font-medium">{example.name}</p>
						<pre className="mt-1 text-xs text-fg-muted font-mono overflow-hidden max-h-[3.6em] leading-[1.2em]">
							{previewLines}
						</pre>
					</button>
				)
