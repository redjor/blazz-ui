...
import { Calendar, Clock, Copy, Download, Euro, Mail, MoreHorizontal, Phone, Send } from "lucide-react"
			...
			<PageWrapper size="lg">
				...
				<div className="flex gap-6 items-start">
						...
						<BlockStack gap="600">
									...
									<table className="w-full text-sm">
										<thead>
											<tr className="border-b border-edge bg-muted/50">
												<th className="px-3 py-2 text-left text-xs font-medium text-fg-muted">Description</th>
												<th className="px-3 py-2 text-right text-xs font-medium text-fg-muted w-20">Qté</th>
												<th className="px-3 py-2 text-right text-xs font-medium text-fg-muted w-28">Prix unit.</th>
												<th className="px-3 py-2 text-right text-xs font-medium text-fg-muted w-28">Total</th>
											</tr>
										</thead>
										<tbody>
											{invoice.lines.map((line) => (
												<tr key={line.desc} className="border-b border-edge last:border-0">
													...
													<td className="px-3 py-2.5 text-right tabular-nums text-fg-muted">{line.qty}j</td>
													<td className="px-3 py-2.5 text-right tabular-nums text-fg-muted">{fmt(line.unit)}</td>
													<td className="px-3 py-2.5 text-right tabular-nums font-medium text-fg">{fmt(line.total)}</td>
												</tr>
											))}
										</tbody>
									</table>
									...
									<div className="border-t border-edge bg-muted/30 px-3 py-3">
										<div className="ml-auto w-56 space-y-1.5">
											<InlineStack align="space-between">
												...
												<span className="text-sm tabular-nums text-fg">{fmt(invoice.amount)}</span>
											</InlineStack>
											<InlineStack align="space-between">
												<span className="text-xs text-fg-muted">TVA 20%</span>
												<span className="text-sm tabular-nums text-fg-muted">{fmt(invoice.tax)}</span>
											</InlineStack>
											<Divider />
											<InlineStack align="space-between">
												<span className="text-sm font-semibold text-fg">Total TTC</span>
												<span className="text-sm font-semibold tabular-nums text-fg">{fmt(invoice.total)}</span>
											</InlineStack>
										</div>
									</div>
								</div>
							...
							<PageSection title="Notes">
								<p className="text-sm text-fg-muted leading-relaxed">
									Mission de développement mars 2026. Facturation au forfait jour. Conditions de paiement : 30 jours date de facture. Pénalités de retard : 3x le taux d'intérêt légal.
								</p>
							</PageSection>
						</BlockStack>
					</div>
					...
					<div className="w-72 shrink-0 space-y-5">
						...
						<div className="rounded-lg border border-edge bg-surface p-4">
							<div className="mb-3 flex items-center gap-2.5">
								<Avatar className="size-8">
									<AvatarFallback className="bg-brand/10 text-brand text-xs">AC</AvatarFallback>
								</Avatar>
						...
						{/* Details card */}
						<div className="rounded-lg border border-edge bg-surface p-4">
							<p className="mb-3 text-xs font-medium uppercase tracking-wider text-fg-muted">Détails</p>
