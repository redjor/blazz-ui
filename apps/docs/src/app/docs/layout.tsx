...
export default function DocsLayout({ children }: { children: ReactNode }) {
	...
	return (
		<div className="flex h-screen flex-col overflow-hidden bg-page">
			<header className="h-14 shrink-0 bg-page z-50">
				<div className="flex h-full items-center px-4">
					<div className="flex items-center gap-2">
						...
						<div className="hidden lg:inline-flex">
							<Link href="/">
								<Image src="/logo_blazz_white.svg" alt="Blazz UI" width={64} height={24} className="hidden h-6 dark:block" />
								<Image src="/logo_blazz_black.svg" alt="Blazz UI" width={64} height={24} className="block h-6 dark:hidden" />
							</Link>
						</div>
					</div>
					...
					<div className="flex items-center gap-1">
						<button type="button" onClick={() => setCommandPaletteOpen(true)} className="inline-flex items-center gap-2 rounded-md p-2 text-fg-muted hover:text-fg hover:bg-muted transition-colors">
			...
			<SidebarProvider style={{ minHeight: 0 }} className="flex-1 gap-2 px-2 pb-2">
				...
				<main className="flex-1 overflow-y-auto min-w-0 bg-background rounded-lg border border-container">{children}</main>
				<DocsMobileSheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen} sectionId={activeSectionId} />
			</SidebarProvider>

			<CommandPalette navigation={navigationConfig} open={commandPaletteOpen} onOpenChange={setCommandPaletteOpen} />
			<Toaster />
		</div>
	)
}
