...
const toasterProps: DocProp[] = [
	{ name: "position", type: '"top-left" | "top-center" | "top-right" | "bottom-left" | "bottom-center" | "bottom-right"', default: '"bottom-right"', description: "Position des toasts à l'écran." },
	{ name: "theme", type: '"light" | "dark" | "system"', default: '"system"', description: "Thème des toasts. Passer resolvedTheme de next-themes pour synchroniser avec le thème de l'app." },
	{ name: "richColors", type: "boolean", default: "true", description: "Active les couleurs sémantiques renforcées pour success, error, warning, info." },
	{ name: "expand", type: "boolean", default: "false", description: "Expand toasts par défaut au lieu de les empiler." },
	{ name: "visibleToasts", type: "number", default: "3", description: "Nombre maximum de toasts visibles simultanément." },
	{ name: "closeButton", type: "boolean", default: "false", description: "Affiche un bouton de fermeture sur chaque toast." },
	{ name: "duration", type: "number", default: "4000", description: "Durée d'affichage en millisecondes." },
	{ name: "offset", type: "number | string", default: "16", description: "Distance par rapport aux bords de l'écran." },
	{ name: "gap", type: "number", default: "8", description: "Espacement entre les toasts empilés." },
]

const toastProgressProps: DocProp[] = [
	{ name: "description", type: "string", description: "Texte secondaire sous le titre." },
	{ name: "duration", type: "number", default: "5000", description: "Durée d'affichage en millisecondes. Contrôle aussi la vitesse de la barre." },
	{ name: "showProgress", type: "boolean", default: "true", description: "Affiche la barre de progression qui se réduit jusqu'à la disparition." },
	{ name: "showCountdown", type: "boolean", default: "true", description: 'Affiche le texte "Dismissing in X seconds" sous le message.' },
]

const toastApiProps: DocProp[] = [
	{ name: "toast(message)", type: "(message: string, options?) => string | number", description: "Toast neutre par défaut." },
	{ name: "toast.success(message)", type: "(message: string, options?) => string | number", description: "Toast de succès (fond vert)." },
	{ name: "toast.error(message)", type: "(message: string, options?) => string | number", description: "Toast d'erreur (fond rouge)." },
	{ name: "toast.warning(message)", type: "(message: string, options?) => string | number", description: "Toast d'avertissement (fond ambre)." },
	{ name: "toast.info(message)", type: "(message: string, options?) => string | number", description: "Toast d'information (fond bleu)." },
	{ name: "toast.loading(message)", type: "(message: string, options?) => string | number", description: "Toast avec spinner de chargement. Persistant jusqu'à dismiss()." },
	{ name: "toast.promise(promise, messages)", type: "(promise: Promise, { loading, success, error }) => string | number", description: "Gère automatiquement les états loading → success/error." },
	...
	return (
		...
		>
			...
			<DocSection id="examples" title="Examples">
				<DocExampleClient title="Default" description="Toast neutre pour les messages simples." code={examples[1].code} highlightedCode={html("default")}>
					<ToastDefaultDemo />
				</DocExampleClient>
				...
				<DocExampleClient title="Avec description" description="Ajoute un sous-texte pour donner plus de contexte." code={examples[3].code} highlightedCode={html("description")}>
					<ToastDescriptionDemo />
				</DocExampleClient>

				<DocExampleClient title="Promise" description="Gère automatiquement les états loading → success/error d'une promesse." code={examples[4].code} highlightedCode={html("promise")}>
					<ToastPromiseDemo />
				</DocExampleClient>

				<DocExampleClient title="Dismiss" description="Contrôle la durée de vie d'un toast via son ID retourné." code={examples[5].code} highlightedCode={html("dismiss")}>
					<ToastDismissDemo />
				</DocExampleClient>
			</DocSection>
