...
import { Item, ItemActions, ItemContent, ItemDescription, ItemFooter, ItemGroup, ItemHeader, ItemMedia, ItemSeparator, ItemTitle } from "@blazz/ui/components/ui/item"
import { Progress } from "@blazz/ui/components/ui/progress"
import { Archive, Bell, Building2, ChevronRight, CreditCard, FileSpreadsheet, FileText, Folder, GitBranch, Image, Inbox, Mail, MoreHorizontal, Send, Shield, Trash2, User } from "lucide-react"
...
const itemMediaProps: DocProp[] = [
	{
		...
		default: '"default"',
		description: "default: no sizing. icon: auto-sizes SVGs to 16px. image: fixed size container (40/32/24px based on Item size) with object-cover.",
	},
]
...
export default function ItemPage() {
	...
	return (
		<DocPage title="Item" subtitle="Flexible list item component for displaying records, contacts, notifications, and any structured row content." toc={toc}>
			{/* Hero */}
			<DocHero>
			...
			<DocSection id="examples" title="Examples">
				...
				<DocExampleClient title="With Icon & Actions" description="Items with icon media and action buttons for settings-style lists." code={examples[1].code} highlightedCode={html("icon-actions")}>
					<div className="max-w-md flex flex-col gap-2">
						<Item variant="outline">
							<ItemMedia variant="icon">
								<Mail />
				...
				{/* 3 — User Follow */}
				<DocExampleClient title="User with Avatar" description="User items with avatar, info, and follow button." code={examples[2].code} highlightedCode={html("user-follow")}>
					<div className="max-w-md flex flex-col gap-2">
						<Item variant="outline" size="xs">
							<ItemMedia>
								<Avatar>
						...
						<Item variant="outline" size="xs">
							<ItemMedia>
								<Avatar>
									<AvatarImage src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=96&h=96&dpr=2&q=80" alt="Sarah Chen" />
									<AvatarFallback>SC</AvatarFallback>
								</Avatar>
							</ItemMedia>
							<ItemContent>
				...
				{/* 5 — Status Badges */}
				<DocExampleClient title="Status Badges" description="Team list with avatar and status badges." code={examples[4].code} highlightedCode={html("status-badges")}>
					<div className="max-w-md">
						<ItemGroup>
							<Item variant="outline" size="xs">
								<ItemMedia>
							...
							<Item variant="outline" size="xs">
								<ItemMedia>
									<Avatar>
										<AvatarImage src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=96&h=96&dpr=2&q=80" alt="David Kim" />
										<AvatarFallback>DK</AvatarFallback>
									</Avatar>
								</ItemMedia>
								<ItemContent>
				...
				{/* 6 — File Items */}
				<DocExampleClient title="File List" description="File items with icon media, metadata, badges, and action buttons." code={examples[5].code} highlightedCode={html("files")}>
					<div className="max-w-md flex flex-col gap-2">
						<Item variant="outline" size="xs">
							<ItemMedia variant="icon">
								<FileText />
				...
				{/* 7 — Mailbox Sidebar */}
				<DocExampleClient title="Mailbox Sidebar" description="Compact navigation items with count badges -- ideal for sidebar menus." code={examples[6].code} highlightedCode={html("mailbox")}>
					<div className="max-w-64 flex flex-col gap-0.5">
						{mailboxItems.map((item) => (
							<Item key={item.label} size="xs" render={<a href="#mailbox" aria-label={item.label} />}>
								<ItemMedia variant="icon">{item.icon}</ItemMedia>
								<ItemContent>
									<ItemTitle>{item.label}</ItemTitle>
								</ItemContent>
				...
				{/* 9 — Project Card */}
				<DocExampleClient title="Project Card" description="Item with header, footer, avatar group, and progress bar." code={examples[8].code} highlightedCode={html("project-card")}>
					<div className="max-w-xs">
						<Item variant="outline">
							...
							<ItemContent>
								<ItemTitle>Website Redesign</ItemTitle>
								<ItemDescription>Complete overhaul of the marketing site with a focus on conversion optimization and modern design patterns.</ItemDescription>
							</ItemContent>
							<ItemActions>
								<Button variant="outline" size="sm">
									Open
				...
				{/* 10 — Integrations */}
				<DocExampleClient title="Integrations" description="Integration items showing connected status or connect action." code={examples[9].code} highlightedCode={html("integrations")}>
					<div className="max-w-md">
						<ItemGroup>
							<Item variant="outline" size="xs">
								<ItemMedia variant="icon">
				...
				<div className="space-y-8">
					...
					<div>
						...
						<p className="text-sm text-fg-muted">
							<code className="text-xs">ItemContent</code>, <code className="text-xs">ItemTitle</code>, <code className="text-xs">ItemDescription</code>, <code className="text-xs">ItemActions</code>,{" "}
							<code className="text-xs">ItemHeader</code>, <code className="text-xs">ItemFooter</code>, and <code className="text-xs">ItemSeparator</code> accept standard div/p props including{" "}
			...
			<DocSection id="guidelines" title="Guidelines">
				<ul className="list-inside list-disc space-y-2 text-sm text-fg-muted">
					<li>
						Use <strong>ItemGroup</strong> to wrap items -- it provides consistent spacing and <code className="text-xs">role="list"</code> for accessibility.
					</li>
					<li>
					...
					<li>
						Use <strong>ItemMedia variant="icon"</strong> for icons and <strong>variant="image"</strong> for avatars/thumbnails. Use default variant (no sizing) when combining with Avatar directly.
					</li>
					<li>
						Use the <strong>render</strong> prop to make items clickable links -- never wrap Item in an anchor.
					</li>
					<li>
						<strong>ItemHeader</strong> and <strong>ItemFooter</strong> span the full width and are ideal for metadata rows with badges, progress bars, or avatar groups.
					</li>
					<li>
						Combine with <strong>Badge</strong> in ItemActions for status indicators, role labels, or count badges.
					</li>
					<li>
						Use <strong>DropdownMenu</strong> in ItemActions for secondary actions on team member or resource lists.
					</li>
