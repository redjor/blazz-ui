...
const appSidebarProps: DocProp[] = [
	{
		...
		required: true,
		description:
			"Configuration de la sidebar : user (name, email, role) + navigation (sections et items).",
	},
	{
		name: "header",
...
function SidebarPlaceholder() {
	return (
		<div className="h-40 w-48 rounded border border-dashed border-edge-subtle bg-muted/50 flex flex-col p-3 gap-2 text-xs text-fg-muted">
			<div className="h-6 rounded bg-card flex items-center px-2">Dashboard</div>
			<div className="h-6 rounded bg-brand/10 flex items-center px-2 text-brand">Contacts ←</div>
			<div className="h-6 rounded bg-card flex items-center px-2">Settings</div>
		</div>
	)
}
...
export default function AppSidebarPage() {
	...
	return (
		<DocPage
			title="App Sidebar"
			subtitle="Sidebar hiérarchique pilotée par un objet SidebarConfig. Supporte les sections, sous-items collapsibles, et le footer utilisateur."
			toc={toc}
		>
