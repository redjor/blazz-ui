...
const toc = [
	...
	{ id: "compatibilite", title: "Compatibilite" },
		...
		>
							...
							{[
								{
									...
									args: "—",
									desc: "Liste les 30+ composants avec nom, categorie et description",
								},
								{
									tool: "get_component",
									args: "name: string",
									desc: "Doc complete d'un composant : props, exemples, gotchas",
								},
								{
									tool: "get_pattern",
									args: "name: string",
								...
								},
								{
									tool: "get_rules",
									args: "—",
									desc: "Conventions de code non negociables du projet",
								},
								{
									tool: "get_design_principles",
									args: "—",
									desc: "Principes de design : Tufte, Gestalt, densite, typographie",
								},
								{
									tool: "get_tokens",
									args: "—",
									desc: "Tokens CSS complets : surfaces, textes, couleurs, densite",
								},
							].map((row) => (
								<tr key={row.tool}>
			...
			<DocSection id="tool-get-component" title="get_component">
				...
				<DocExampleClient
					...
					description="La section Markdown complete du composant, extraite automatiquement."
					...
					<div className="space-y-2">
						...
						<div className="grid gap-2 sm:grid-cols-2">
							{[
								...
								"4 etats : loading, empty, error, success",
					...
					<div className="space-y-2">
						...
						<div className="grid gap-2 sm:grid-cols-2">
							{[
								...
								"Gestalt : proximite, similarite",
								"Densite : 13px, 40px rows",
								"Espacement : echelle de 4px",
								"Typographie : Inter, 3 niveaux",
								"Couleurs : semantiques uniquement",
							].map((p) => (
			...
			<DocSection id="workflow" title="Workflow type">
				...
				<DocExampleClient
					...
					description="L'IA combine les tools pour generer du code conforme au design system."
					code={examples[8].code}
					highlightedCode={html("workflow-example")}
				>
					<div className="space-y-3">
						{[
							{ step: "1", label: "get_rules", desc: "Charger les conventions au demarrage" },
							{ step: "2", label: "get_pattern", desc: "Obtenir le skeleton de la page demandee" },
							{
								step: "3",
								label: "get_component",
								desc: "Recuperer les props des composants utilises",
							},
			...
			{/* Compatibilite */}
			<DocSection id="compatibilite" title="Compatibilite">
				<div className="overflow-hidden rounded-lg border border-container">
					<table className="w-full text-sm">
						<thead>
							<tr className="border-b border-separator bg-muted/50">
						...
						<tbody className="divide-y divide-separator">
							{[
								{ client: "Claude Code", config: ".claude/mcp.json", status: "Supporte" },
								{
									client: "Claude Desktop",
									config: "claude_desktop_config.json",
									status: "Supporte",
								},
								{ client: "Cursor", config: ".cursor/mcp.json", status: "Supporte" },
								{ client: "Windsurf", config: ".windsurf/mcp.json", status: "Supporte" },
								{ client: "Continue", config: ".continue/config.json", status: "Supporte" },
							].map((row) => (
								<tr key={row.client}>
				...
				<DocRelated
					items={[
						...
						},
						{
							...
							description: "Le composant DataTable documente via le MCP.",
						},
					]}
				/>
			</DocSection>
