...
function findActiveParentItemId(navigation: NavigationSection[], pathname: string): string | null {
	for (const section of navigation) {
		for (const item of section.items) {
			...
			if (item.items) {
				const hasActiveSubItem = item.items.some((subItem) => subItem.url && pathname.startsWith(subItem.url))
...
export function DocsSidebar({ sectionId }: { sectionId: SectionId }) {
	...
	return (
		<Sidebar collapsible="none" className="hidden lg:flex rounded-lg overflow-hidden">
			<SidebarContent>
				<NavSection section={{ ...section, title: undefined }} isActive={isActive} openItemId={openItemId} setOpenItemId={setOpenItemId} />
			</SidebarContent>
		</Sidebar>
	)
}
			...
			<SidebarGroupContent>
				<SidebarMenu>
					{section.items.map((item) => (
						<NavItem key={item.id ?? item.url ?? item.title} item={item} isActive={isActive} openItemId={openItemId} setOpenItemId={setOpenItemId} />
					))}
				</SidebarMenu>
			</SidebarGroupContent>
		</SidebarGroup>
...
function NavItem({ item, isActive, openItemId, setOpenItemId }: { item: NavigationItem; isActive: (url?: string) => boolean; openItemId: string | null; setOpenItemId: (id: string | null) => void }) {
