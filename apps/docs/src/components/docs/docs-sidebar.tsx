...
import type {
	NavigationItem,
	NavigationSection,
} from "@blazz/ui/types/navigation";
...
function findActiveParentItemId(
...
export function DocsSidebar({ sectionId }: { sectionId: SectionId }) {
  ...
  return (
    <Sidebar
      collapsible="none"
      className="hidden lg:flex rounded-lg overflow-hidden"
    >
      <SidebarContent>
        <NavSection
...
function NavItem({
  ...
  const hasActiveChild =
    hasChildren && item.items?.some((sub) => isActive(sub.url));
