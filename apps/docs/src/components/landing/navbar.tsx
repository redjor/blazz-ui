	...
	return (
		<header
			className={`sticky top-0 z-50 h-13 shrink-0 transition-[background-color,border-color] duration-200 ${
				scrolled ? "bg-app/95 backdrop-blur-md border-b border-edge/50" : "bg-app border-b border-transparent"
			}`}
		>
			<div className="mx-auto flex h-full max-w-6xl items-center gap-6 px-6">
				...
				<nav className="hidden md:flex items-center gap-1">
					{navLinks.map((link) => (
						<Link key={link.href} href={link.href} className="rounded-md px-3 py-1.5 text-[13px] text-fg-muted transition-colors hover:text-fg hover:bg-muted">
							{link.label}
						</Link>
					))}
				</nav>
				...
				<div className="flex items-center gap-1">
					<ThemeToggle />
					<Link href="/docs/components" className="inline-flex items-center gap-2 rounded-md px-2.5 py-1.5 text-[13px] text-fg-muted transition-colors hover:text-fg hover:bg-muted">
