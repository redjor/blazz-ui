...
function ColumnsPage() {
	const readOnlyColumns = [
		{
			name: "col.text",
			signature: `col.text<T>("name", { showInlineFilter: true })`,
			desc: "Texte brut avec filtre text optionnel",
		},
		{
			name: "col.numeric",
			signature: `col.numeric<T>("stock", { title: "Stock" })`,
			desc: "Nombre aligne a droite, filtre number",
		},
		{
			name: "col.currency",
			signature: `col.currency<T>("price", { currency: "EUR", locale: "fr-FR" })`,
			desc: "Montant formate avec devise et locale",
		},
		{
			name: "col.date",
			signature: `col.date<T>("createdAt", { format: "dd/MM/yyyy" })`,
			desc: "Date formatee avec locale",
		},
		{
			name: "col.relativeDate",
			signature: `col.relativeDate<T>("updatedAt")`,
			desc: 'Date relative — "Il y a 2h", "Dans 3 jours"',
		},
		{
			name: "col.status",
			signature: `col.status<T>("status", { statusMap: { active: { label: "Actif", variant: "success" } } })`,
			desc: "Badge colore selon une map de statuts",
		},
		{
			name: "col.select",
			signature: `col.select<T>("category", { options: [{ label: "...", value: "..." }] })`,
			desc: "Texte avec filtre dropdown",
		},
		{
			name: "col.boolean",
			signature: `col.boolean<T>("isActive")`,
			desc: "Checkbox, badge ou icone vrai/faux",
		},
		{
			name: "col.tags",
			signature: `col.tags<T>("labels")`,
			desc: "Tableau de strings affiche en badges inline",
		},
		{
			name: "col.progress",
			signature: `col.progress<T>("completion")`,
			desc: "Barre de progression 0-100",
		},
		{
			name: "col.rating",
			signature: `col.rating<T>("score", { max: 5 })`,
			desc: "Etoiles ou dots pour une note",
		},
		{
			name: "col.link",
			signature: `col.link<T>("website")`,
			desc: "Lien cliquable (URL, email, telephone)",
		},
		{
			name: "col.user",
			signature: `col.user<T>("assignee")`,
			desc: "Avatar circulaire + nom + sous-titre optionnel",
		},
		{
			name: "col.duration",
			signature: `col.duration<T>("timeSpent")`,
			desc: 'Duree lisible — "2h 30m", "45s"',
		},
		{
			name: "col.colorDot",
			signature: `col.colorDot<T>("priority")`,
			desc: "Dot colore + label texte",
		},
		{
			name: "col.image",
			signature: `col.image<T>("thumbnail")`,
			desc: "Miniature image avec bords arrondis",
		},
		{
			name: "col.sparkline",
			signature: `col.sparkline<T>("trend")`,
			desc: "Mini graphique SVG inline (line ou bar)",
		},
		{
			name: "col.twoLines",
			signature: `col.twoLines<T>("title", { subtitleAccessor: "subtitle" })`,
			desc: "Ligne principale + sous-titre muted",
		},
		{
			name: "col.keyValue",
			signature: `col.keyValue<T>("metadata")`,
			desc: 'Paire "label: value" inline',
		},
		{
			name: "col.imageText",
			signature: `col.imageText<T>("name", { imageAccessor: "avatar" })`,
			desc: "Image + texte cote a cote",
		},
		{
			name: "col.avatarGroup",
			signature: `col.avatarGroup<T>("members")`,
			desc: "Avatars circulaires superposes",
		},
	]

	const editableColumns = [
		{
			name: "col.editableText",
			signature: `col.editableText<T>("name", { onCellEdit })`,
			desc: "Texte editable au clic ou F2",
		},
		{
			name: "col.editableNumber",
			signature: `col.editableNumber<T>("price", { onCellEdit })`,
			desc: "Nombre editable avec validation",
		},
		{
			name: "col.editableCurrency",
			signature: `col.editableCurrency<T>("amount", { onCellEdit, currency: "EUR" })`,
			desc: "Montant editable avec formatage devise",
		},
		{
			name: "col.editableSelect",
			signature: `col.editableSelect<T>("status", { onCellEdit, options: [...] })`,
			desc: "Dropdown editable avec options",
		},
		{
			name: "col.editableDate",
			signature: `col.editableDate<T>("dueDate", { onCellEdit })`,
			desc: "Date editable avec date picker",
		},
	]
	...
	const specialColumns = [
		{
			name: "col.selection",
			signature: `col.selection<T>()`,
			desc: "Checkbox de selection (header = select-all, cell = par ligne)",
		},
		{
			name: "col.expand",
			signature: `col.expand<T>()`,
			desc: "Chevron pour expand/collapse le detail panel",
		},
	]
