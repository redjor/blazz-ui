...
import { FilterPanel, FilterPanelAction, FilterPanelActions, FilterPanelCheckboxItem, FilterPanelHeader, FilterPanelSection, FilterPanelTabs, FilterPanelTreeItem } from "@blazz/ui/components/ui/filter-panel"
	...
	return (
		<FilterPanel>
			<FilterPanelHeader>
				...
				<FilterPanelActions>
					<FilterPanelAction aria-label="Search"><Search className="size-4" /></FilterPanelAction>
					<FilterPanelAction aria-label="Settings"><Settings2 className="size-4" /></FilterPanelAction>
				</FilterPanelActions>
			</FilterPanelHeader>
			<FilterPanelSection label="Market type">
				<FilterPanelTreeItem icon={<Repeat className="size-3.5" />} label="Perpetuals" count={45}>
					<FilterPanelCheckboxItem icon={<BarChart3 className="size-3.5" />} label="Open Interest" count={12} depth={1} checked={checked["Open Interest"] ?? false} onCheckedChange={toggle("Open Interest")} />
					<FilterPanelCheckboxItem icon={<Percent className="size-3.5" />} label="Funding Rates" count={8} depth={1} checked={checked["Funding Rates"] ?? false} onCheckedChange={toggle("Funding Rates")} />
				</FilterPanelTreeItem>
				<FilterPanelCheckboxItem icon={<Activity className="size-3.5" />} label="High Volatility" count={10} checked={checked["High Volatility"] ?? false} onCheckedChange={toggle("High Volatility")} />
				...
				<FilterPanelCheckboxItem icon={<CircleOff className="size-3.5" />} label="No Active Market" count={8} checked={checked["No Active Market"] ?? false} onCheckedChange={toggle("No Active Market")} />
			</FilterPanelSection>
		</FilterPanel>
	)
}
