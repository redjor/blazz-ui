...
export function createContactsPreset(config: ContactsPresetConfig = {}): ContactsPreset {
	...
	const columns: DataTableColumnDef<Contact>[] = [
		...
		col.text<Contact>("companyName", {
			...
			cellRenderer: (_value, row) => (
				<Link href={`/companies/${row.companyId}`} className="text-fg hover:underline">
					{row.companyName}
				</Link>
			),
		}),
	...
	const bulkActions = createBulkActions<Contact>({
		...
		archiveConfirmation: (count) => `Êtes-vous sûr de vouloir archiver ${count} contact(s) ?`,
		deleteConfirmation: (count) => `Êtes-vous sûr de vouloir supprimer ${count} contact(s) ?`,
		labels: {
