...
import { addDays, format, isFriday, isThisWeek, isToday, isTomorrow, nextFriday, nextMonday, parseISO } from "date-fns"
			...
			<PopoverTrigger
				render={
					<Button
						...
						className={compact ? "h-7 px-2 text-xs text-fg-muted font-normal" : "w-full justify-start text-left font-normal"}
					/>
				}
			>
				<Calendar className={compact ? "size-3.5 text-fg-muted" : "size-4 text-fg-muted mr-2"} />
				{displayLabel ? <span className={compact ? "text-xs" : "text-sm"}>{displayLabel}</span> : <span className={compact ? "text-xs text-fg-muted" : "text-sm text-fg-muted"}>Échéance</span>}
			</PopoverTrigger>
			<PopoverContent className="w-auto !p-1" align="start">
				<div className="flex">
					...
					<div className="flex flex-col w-[200px] p-1">
						{shortcuts.map((s) => (
							<button key={s.label} type="button" onClick={() => pick(s.getDate())} className="flex items-center gap-3 w-full px-3 py-2 text-sm rounded-md hover:bg-muted transition-colors">
								{s.icon}
						...
						{value && (
							<>
								...
								<button type="button" onClick={clear} className="flex items-center gap-3 w-full px-3 py-2 text-sm rounded-md hover:bg-muted transition-colors">
