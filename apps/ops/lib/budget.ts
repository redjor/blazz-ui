...
export function computeBudgetMetrics(opts: { budgetAmount: number | undefined; tjm: number; hoursPerDay: number; billableMinutes: number; billableRevenue: number }): BudgetMetrics | null {
	...
	const daysConsumed = opts.hoursPerDay > 0 ? opts.billableMinutes / (opts.hoursPerDay * 60) : 0
	const remaining = opts.budgetAmount - opts.billableRevenue
	const percentUsed = daysSold > 0 ? (daysConsumed / daysSold) * 100 : 0
	const effectiveTjm = daysConsumed > 0 ? opts.billableRevenue / daysConsumed : null
