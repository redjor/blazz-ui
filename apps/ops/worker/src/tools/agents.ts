	...
	return [
		...
		},
		{
			...
			execute: async (args) => {
				...
				const agentTools = allTools.filter((t) => t.category === "read" && targetAgent.permissions.safe.includes(t.name))

				const toolDefs = agentTools.length > 0 ? agentTools.map((t) => t.definition) : undefined
		...
		},
		{
			...
			execute: async (args, convex) => {
				...
				const _agents = await convex.query(api.worker.workerListByStatus, { status: "todo" })
				...
				const expiresAt =
					category === "fact" || category === "episode"
